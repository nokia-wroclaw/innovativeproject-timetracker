...
public class PrivilegesStorage {
    ...
    public static List<SendedObject> getYou(String login) {
        ...
        while (iterator.hasNext()) {
            ...
            int minutes = models.TimeStorage.getWorkedMinutes(item.userto);

            SendedObject obj =
                    new SendedObject(id, usernameandsurname.name, usernameandsurname.surname, item.userfrom,
                            item.userto, item.estimatedHours, minutes);
            finallist.add(obj);
            iterator.next();
            id++;
        }
